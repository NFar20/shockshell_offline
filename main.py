import pygame
import math
from pygame.locals import (K_UP, K_DOWN, K_LEFT, K_RIGHT, K_SPACE, K_ESCAPE,
                           KEYDOWN, K_MINUS, K_EQUALS)
...
class Tank(pygame.sprite.Sprite):
  ...
  def launch(self):
    ...
    currentProjectile = Projectile(
      self.x, self.y,
      self.launchSpeed * math.cos(self.launchAngle * math.pi / 180),
...
def angleText():
  ...
  redAngle = font.render(
    str(180 - playerTwo.launchAngle) + "\u00B0", True, red)
...
def strengthText():
  ...
  blueStrength = font.render(
    str((playerOne.launchSpeed - 20) * 5) + "%", True, blue)
  redStrength = font.render(
    str((playerTwo.launchSpeed - 20) * 5) + "%", True, red)
...
def update():
  ...
  if currentProjectile != None:
    screen.blit(currentProjectile.image,
                (currentProjectile.x, currentProjectile.y))
  if currentProjectile != None:
